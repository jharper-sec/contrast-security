...
func testJavaAgent(t *testing.T, context spec.G, it spec.S) {
	...
	it("contributes Java agent", func() {
		dep := libpak.BuildpackDependency{
			ID:     "contrast-java",
			URI:    "https://localhost/stub-contrast-java-agent.jar",
			SHA256: "ee23306ce5f7086219c1876652ed323970ebc249f21d1c79b737ac1120284bbf",
		}
		dc := libpak.DependencyCache{CachePath: "testdata"}

		j, _ := contrast.NewJavaAgent(ctx.Buildpack.Path, dep, dc)
		...
		Expect(filepath.Join(layer.Path, "stub-contrast-java-agent.jar")).To(BeARegularFile())
		...
		Expect(layer.LaunchEnvironment["JAVA_TOOL_OPTIONS.append"]).To(Equal(fmt.Sprintf("-javaagent:%s",
			filepath.Join(layer.Path, "stub-contrast-java-agent.jar"))))
	})
}
